...
class AgentSpecPipelineCompletionPropertyTest {
    ...
    void pipelineApprovedUpdatesStatusToApproved(
        ...
        // Capture the callback from execute(3-arg) and return the caller-provided executionId
        ArgumentCaptor<PipelineCallback> callbackCaptor = ArgumentCaptor.forClass(PipelineCallback.class);
        when(publishPipelineExecutor.execute(
                any(PublishPipelineContext.class), callbackCaptor.capture(), any(String.class)))
