...
class PromptDataMigrationTaskTest {
    ...
    void testShouldSkipWhenMarkerAcquireFails() throws Exception {
        ...
        // Marker is not stale
        ConfigQueryChainResponse markerResponse = new ConfigQueryChainResponse();
        markerResponse.setStatus(ConfigQueryChainResponse.ConfigQueryStatus.CONFIG_FOUND_FORMAL);
        markerResponse.setContent(String.valueOf(System.currentTimeMillis()));
        when(configQueryChainService.handle(any())).thenReturn(markerResponse);
    ...
    void testShouldReleaseMigrationMarkerEvenOnFailure() throws Exception {
        ...
        // Marker creation succeeds
        // Config read fails during migration
        when(configQueryChainService.handle(any())).thenThrow(new RuntimeException("config read failure"));
